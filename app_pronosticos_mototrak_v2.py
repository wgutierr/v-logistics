...
from funciones_pronosticos.funciones_pronosticos_nixtla import ejecutar_motor_statsforecast
...
if seccion == "Pronósticos PT":
    ...
    if archivo_norte and archivo_centro and archivo_sur:
        ...
        if st.button("Generar pronóstico de PT"):
            ...
            score_global_pt = res_pt["score_global"]     # KPI general (1 fila)
            score_pt = res_pt["score_por_serie"]         # 👈 para merge (por unique_id)

            ganadores_pt = res_pt["mejores_modelos"]
            forecast_pt = res_pt["forecasts_filtrados"]

            st.session_state["df_resumen_pt"] = ganadores_pt.merge(score_pt, on="unique_id", how="left")
            st.session_state["score_global_pt"] = score_global_pt


            st.subheader("Score global PT (promedio)")
            st.dataframe(st.session_state["score_global_pt"], use_container_width=True)
            # Si quieres mantener tu “resumen_pt.xlsx” similar al anterior,
            # aquí lo más directo es exportar ganadores + score:
            # --- Normalizar columnas clave para merge ---
            ganadores_pt_ = ganadores_pt.copy()
            score_pt_ = score_pt.copy()

            # Si unique_id viene como índice, lo volvemos columna
            if "unique_id" not in ganadores_pt_.columns and ganadores_pt_.index.name == "unique_id":
                ganadores_pt_ = ganadores_pt_.reset_index()

            if "unique_id" not in score_pt_.columns and score_pt_.index.name == "unique_id":
                score_pt_ = score_pt_.reset_index()

            # Si aún no existe, mostramos columnas para diagnóstico
            if "unique_id" not in ganadores_pt_.columns or "unique_id" not in score_pt_.columns:
                st.error("No encuentro la columna 'unique_id' en ganadores_pt o score_pt.")
                st.write("Columnas ganadores_pt:", list(ganadores_pt_.columns))
                st.write("Index ganadores_pt:", ganadores_pt_.index.name)
                st.write("Columnas score_pt:", list(score_pt_.columns))
                st.write("Index score_pt:", score_pt_.index.name)
            else:
                st.session_state["df_resumen_pt"] = ganadores_pt_.merge(score_pt_, on="unique_id", how="left")
    ...
    if 'df_resumen_pt' in st.session_state:
        ...
        if "score_pt" in st.session_state:
            st.subheader("Score de modelos (PT)")
            st.dataframe(st.session_state["score_pt"], use_container_width=True)
